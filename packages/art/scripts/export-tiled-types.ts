...
const parseWangset = (wangset: xlmJs.ElementCompact): WangSet => {
  ...
  const wangtileObjects = Array.isArray(wangtiles) 
    ? wangtiles.map(parseWangtile) 
    : [parseWangtile(wangtiles)];
  return {
    name,
...
const tilemapObjects = tilemaps.map((filename, index) => processTilemap(filename));
