...
const TOKENS_TO_LOAD = ["SPHR", "Blue"]; // Ensure these are unique

export class StreamStore {
  realtimeBalances = new Map<string, RealTimeBalance>(); // Map to ensure unique keys
  wallet: string;
  framework: Framework;
  provider: Provider;
  ...
  async init() {
    return Promise.all(
      TOKENS_TO_LOAD.map((token) => this.loadRealTimeBalance(token))
    );
  }
