...
export const createPhaserLayer = async (
  ...
  // Add event listener to resume AudioContext
  const handleUserGesture = () => {
    const sound = scenes.Main.phaserScene.sound;
    if (sound instanceof Phaser.Sound.WebAudioSoundManager && sound.context.state !== 'running') {
      sound.context.resume();
    }
  };
  window.addEventListener('click', handleUserGesture);

  // Cleanup event listener when disposed
  const originalDispose = layer.dispose;
  layer.dispose = () => {
